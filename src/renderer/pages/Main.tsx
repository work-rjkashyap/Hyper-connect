                ...
                <div className="px-4 pb-4">
                    <div className="p-3 bg-secondary/50 rounded-lg border border-border/50">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-xs text-muted-foreground font-medium uppercase tracking-wider">My Device</p>
                        ...
                        discoveredDevices.map(device => (
                            ...
                            >
                                <div className={cn(
                                    ...
                                    {device.platform === 'darwin' ? <Laptop className="w-4 h-4" /> : <Monitor className="w-4 h-4" />}
                                </div>
                                <div className="text-left flex-1 min-w-0">
                                    <p className="text-sm font-semibold truncate">{device.displayName}</p>
                                    <p className={cn(
                                        "text-xs truncate",
                                        selectedDeviceId === device.deviceId ? "text-primary-foreground/80" : "text-muted-foreground"
                                    )}>
                                        {device.address}
                                    </p>
                                </div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            <header className="h-16 border-b bg-background/80 backdrop-blur-md px-6 flex items-center justify-between shrink-0">
                ...
                <div className="flex bg-secondary p-1 rounded-lg">
                    <button
                        onClick={() => setTab('chat')}
                        className={cn("px-4 py-1.5 text-sm font-semibold rounded-md transition-all", tab === 'chat' ? "bg-background shadow-sm" : "hover:text-primary")}
                    >
                        Chat
                    </button>
                    <button
                        onClick={() => setTab('files')}
                        className={cn("px-4 py-1.5 text-sm font-semibold rounded-md transition-all", tab === 'files' ? "bg-background shadow-sm" : "hover:text-primary")}
                    >
                        Files
                    </button>
                </div>
            ...
            {tab === 'chat' ? (
                <div className="flex-1 flex flex-col min-h-0 bg-background/30 shadow-inner">
                    <div className="flex-1 overflow-y-auto p-6 space-y-4">
                        ...
                        {messages.map((msg, i) => (
                            <div key={i} className={cn("flex flex-col animate-in fade-in slide-in-from-bottom-2 duration-300", msg.deviceId === device.deviceId ? "items-start" : "items-end")}>
                                <div className={cn(
                    ...
                    <div className="p-4 bg-background border-t">
                        <div className="flex gap-2 max-w-4xl mx-auto items-end">
                            ...
                            <Button size="icon" className="h-[46px] w-[46px] rounded-xl shadow-lg shadow-primary/20" onClick={handleSend}>
...
const TransferView: React.FC<{ device: Device }> = ({ device }) => {
    const transfers = useStore(state => Object.values(state.transfers).filter(t => t.deviceId === device.deviceId))
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-background/50">
            ...
            >
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">Select a file from your computer to transfer instantly.</p>
                </div>
                <Button variant="outline" className="mt-2 font-semibold">Choose File</Button>
            </div>

            <div className="space-y-4">
                <h3 className="text-sm font-bold uppercase tracking-widest text-muted-foreground/60 px-2 flex items-center gap-2">
                    <Separator className="flex-1 opacity-20" /> Recent Transfers <Separator className="flex-1 opacity-20" />
                </h3>
                {transfers.length === 0 ? (
                                ...
                                <CardContent className="p-4 flex items-center gap-4">
                                    ...
                                    <div className="flex-1 min-w-0 space-y-2">
                                        <div className="flex items-center justify-between gap-4">
                                            <p className="text-sm font-bold truncate">{(transfer as any).name || 'File'}</p>
                                        ...
                                        {transfer.status === 'active' && (
                                            <div className="space-y-1.5 animate-in fade-in duration-500">
                                                <div className="flex justify-between text-[10px] text-muted-foreground font-medium uppercase tracking-tighter">
                                                    <span>{(transfer.progress * 100).toFixed(0)}% • {(transfer.speed / 1024 / 1024).toFixed(1)} MB/s</span>
