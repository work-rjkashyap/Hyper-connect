...
import type { Device, FlushResult } from "@/types";
			...
			try {
				...
				unlistenDiscovered = await listen<Device>(
					...
					(event) => {
						...
						// Auto-flush any queued messages for this device (offline retry)
						invoke<FlushResult>("flush_message_queue", {
							deviceId: event.payload.device_id,
						})
							.then((result) => {
								if (result.flushed > 0) {
									console.log(
										`📬 Flushed ${result.flushed} queued message(s) for ${event.payload.device_id} (${result.remaining} remaining)`,
									);
								}
							})
							.catch(() => {
								// Silently ignore — device may have disappeared before flush
							});
					},
				);
